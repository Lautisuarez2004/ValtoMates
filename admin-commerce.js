(() => {
  ...
  async function syncProductDiscountPrices(transferPct,cashPct){
    const {data,error}=await sb.from('valto_products').select('id,price').eq('visible',true);
    if(error)throw error;
    for(const p of data||[]){
      const price=Number(p.price||0);
      const transfer=Math.round(price*(1-transferPct/100)*100)/100;
      const cash=Math.round(price*(1-cashPct/100)*100)/100;
      const {error:uErr}=await sb.from('valto_products').update({transfer_price:transfer,cash_price:cash,updated_at:new Date().toISOString()}).eq('id',p.id);
      if(uErr)throw uErr;
    }
  }
  ...
  async function saveSettings(){
    ...
    toast('Envíos, descuentos, precios y cuotas actualizados');
  }
  ...
  async function renderOrders(){
    ...
    if(error){console.error(error);return;}
    const rows=data||[];
    target.innerHTML=rows.length?rows.map(o=>{const items=(o.valto_order_items||[]).map(i=>`${esc(i.name_snapshot)}${i.variant?` · ${esc(i.variant)}`:''} ×${i.quantity}`).join('<br>')||'—';const customer=[o.customer_first_name,o.customer_last_name].filter(Boolean).join(' ');const meta=`<small style="display:block;color:var(--muted);margin-top:5px">${esc(methodLabel(o.payment_method))} · ${esc(shippingLabel(o.shipping_method))}${o.postal_code?` · CP ${esc(o.postal_code)}`:''}${customer?`<br>${esc(customer)}`:''}${o.contact_email?` · ${esc(o.contact_email)}`:''}</small>`;const paid=o.payment_status==='approved'||o.status==='paid';const failed=['payment_failed','rejected','cancelled'].includes(o.status)||o.payment_status==='rejected';const cls=paid?'paid':failed?'failed':'pending';const label=paid?'Aprobado':o.status==='awaiting_transfer'?'Esperando transferencia':o.status==='awaiting_cash'?'Pago en efectivo':o.status==='seller_coordination'?'A coordinar':failed?'Fallido':o.payment_status||'Pendiente';return `<tr><td>${new Date(o.created_at).toLocaleString('es-AR')}</td><td>${items}${meta}</td><td>${fmt(o.total)}</td><td><span class="order-status ${cls}">${esc(label)}</span></td><td>${o.stock_committed?'Descontado':'Pendiente'}</td><td style="font-size:11px">${esc(String(o.id).slice(0,8))}</td></tr>`;}).join(''):'<tr><td colspan="6">Todavía no hay ventas registradas.</td></tr>';
  }
