(() => {
  ...
  function injectStyles(){
    ...
    style.textContent = `
      ...
      .payment-btn{width:100%;background:#009ee3!important;color:#fff!important;border-color:#009ee3!important}.payment-btn:disabled{opacity:.6;cursor:wait}
  ...
  function enhanceModal(){
    ...
    const p = getState().products?.find(x => x.id === currentProductId);
    if(!p) return;
    ...
    box.innerHTML = `
      ...
      ${variants.length ? `<select class="payment-variant" id="paymentVariant"><option value="">Elegí una opción</option>${variants.map(v=>`<option value="${esc(v)}">${esc(v)}</option>`).join('')}</select>` : ''}
      <button class="btn payment-btn" id="payOnlineBtn">Pagar con Mercado Pago o tarjeta</button>
      <p class="payment-note">El pago se procesa en Mercado Pago. Podés usar tarjeta de crédito, débito o saldo de Mercado Pago según disponibilidad.</p>`;
    const wa = inner.querySelector('#modalWa');
    wa?.parentElement?.insertBefore(box, wa);
    $('#payOnlineBtn').onclick = () => pay(p, variants.length > 0);
  }

  async function pay(p, requiresVariant){
    const btn = $('#payOnlineBtn');
    const variant = $('#paymentVariant')?.value || '';
    ...
    if(paymentId && result === 'success'){
      try{
        ...
        if(r.ok){
          ...
          text = kind === 'approved' ? `Pago aprobado por ${fmt(data.amount)}. ¡Gracias por tu compra!` : kind === 'pending' ? 'El pago está siendo procesado por Mercado Pago.' : 'Mercado Pago informó que el pago no fue aprobado.';
        }
      }catch{}
    }
  ...
  document.addEventListener('click', (e) => {
    ...
    if(el?.dataset?.open){ currentProductId = el.dataset.open; setTimeout(enhanceModal, 0); }
  }, true);
  ...
  if(footerStatus) footerStatus.textContent = 'Pagos online con Mercado Pago · Crédito y débito.';
  showReturnStatus();
})();
