(() => {
  ...
  function toast(msg){const t=$('#checkoutToast');t.textContent=msg;t.classList.add('show');setTimeout(()=>t.classList.remove('show'),2600);}
  ...
  async function loadRemote(){
    ...
    const [sr,pr]=await Promise.all([
      ...
      fetch(`${config.supabaseUrl}/rest/v1/valto_products?id=in.(${encodeURIComponent(filter)})&select=id,name,price,transfer_price,cash_price,stock,image,variants,variant_label,visible`,{headers,cache:'no-store'})
    ]);
  ...
  function subtotal(method='mercadopago'){return cart.reduce((sum,i)=>{const p=product(i.productId);return sum+(p?priceFor(p,method)*Number(i.quantity||0):0);},0);}
  ...
  function renderShipping(){
    ...
    const branch=Number(settings.shipping_branch_cost||0),home=Number(settings.shipping_home_cost||0);
    $('#shippingOptions').innerHTML=`
      <label class="choice"><input type="radio" name="shippingMethod" value="correo_sucursal"><div><div class="choice-title">Correo Argentino · Retiro por sucursal</div><div class="choice-copy">${eta}${cp?' · CP '+esc(cp):''}</div></div><div class="choice-price">${fmt(branch)}</div></label>
      <label class="choice"><input type="radio" name="shippingMethod" value="correo_domicilio"><div><div class="choice-title">Correo Argentino · Envío a domicilio</div><div class="choice-copy">${eta}${cp?' · CP '+esc(cp):''}</div></div><div class="choice-price">${fmt(home)}</div></label>
    ...
    $$('input[name="shippingMethod"]').forEach(r=>r.onchange=()=>{toggleAddress();renderOrderDetails();saveDraft();});
  }
  ...
  function renderPayments(){
    ...
    $$('input[name="paymentMethod"]').forEach(r=>r.onchange=()=>{renderFinalSummary();renderOrderDetails();saveDraft();});
  }
  ...
  function renderFinalSummary(){
    ...
    $('#finalSummary').innerHTML=`<div class="summary-row"><span>Productos</span><b>${fmt(sub)}</b></div>${disc?`<div class="summary-row"><span>Descuento</span><b>− ${fmt(disc)}</b></div>`:''}<div class="summary-row"><span>Envío</span><b>${selectedShipping()==='coordinar'?'A coordinar':fmt(ship)}</b></div><div class="summary-row total"><span>Total</span><span>${fmt(sub+ship)}</span></div>`;
  ...
  function showStep(step){
    ...
    if(step===3){renderPayments();restoreSelections();renderFinalSummary();}
    window.scrollTo({top:0,behavior:'smooth'});
  }

  function readCheckout(){return {
  ...
  async function init(){
    ...
    renderShipping();restoreSelections();renderPayments();restoreSelections();renderOrderDetails();renderFinalSummary();
    $('#postalMirror').value=postal();
  }

  $('#orderToggle').onclick=()=>$('#orderDetails').classList.toggle('hidden');
  $('#toShipping').onclick=()=>{if(validateStep1()){saveDraft();showStep(2);}};
  $('#toPayment').onclick=()=>{if(validateStep1()&&validateShipping()){saveDraft();showStep(3);}};
  $('#finishOrder').onclick=finish;
  $('#checkoutPostal').addEventListener('input',()=>{const cp=postal();$('#checkoutPostal').value=cp;$('#postalMirror').value=cp;renderShipping();restoreSelections();saveDraft();});
  $$('[data-step-go]').forEach(b=>b.onclick=()=>{const n=Number(b.dataset.stepGo);if(n===2&&!validateStep1())return;if(n===3&&(!validateStep1()||!validateShipping()))return;saveDraft();showStep(n);});
