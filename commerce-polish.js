(() => {
  ...
  function polishVariants(root=document){
    root.querySelectorAll?.('.variant-selector').forEach(box=>{
      ...
      if(!select) return;
      box.dataset.pillsReady='1';select.style.display='none';
      const pills=document.createElement('div');pills.className='variant-pills';
      [...select.options].filter(o=>o.value).forEach(opt=>{const b=document.createElement('button');b.type='button';b.className='variant-pill'+(select.value===opt.value?' active':'');b.textContent=opt.textContent;b.onclick=()=>{select.value=opt.value;select.dispatchEvent(new Event('change',{bubbles:true}));pills.querySelectorAll('.variant-pill').forEach(x=>x.classList.toggle('active',x===b));};pills.appendChild(b);});
  ...
  function addPostalHelp(root=document){
    root.querySelectorAll?.('.shipping-box').forEach(box=>{
      if(!box.querySelector('.postal-help-link')){const result=box.querySelector('.shipping-result');if(result){const a=document.createElement('a');a.className='postal-help-link';a.href=CPA_URL;a.target='_blank';a.rel='noopener';a.textContent='No sé mi código postal';result.insertAdjacentElement('afterend',a);}}
      const btn=box.querySelector('#productShippingCalc,#cartShippingCalc');
      if(btn&&btn.dataset.correoQuoteReady!=='1'){
        btn.dataset.correoQuoteReady='1';
        btn.addEventListener('click',()=>setTimeout(()=>{
          const settings=window.VALTO_COMMERCE_SETTINGS||{};
          if(settings.shipping_provider!=='correo_argentino')return;
          const input=box.querySelector('input');const cp=String(input?.value||'').trim();if(!/^[A-Z0-9]{4,8}$/i.test(cp))return;
          const result=box.querySelector('.shipping-result');
          if(result)result.textContent=`Correo Argentino · Sucursal ${money(settings.shipping_branch_cost)} · Domicilio ${money(settings.shipping_home_cost)}`;
          const help=box.querySelector('.shipping-help');if(help)help.textContent='Tarifas configuradas por Valto. La cotización automática exacta se activa al conectar las credenciales de Correo Argentino.';
        },0),true);
      }
    });
  }
