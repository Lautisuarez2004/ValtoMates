...
export default async (req: Request) => {
  ...
  try {
    const sync = await fetch(COMMERCE_URL, {
      ...
      body: JSON.stringify({ action: 'sync_payment', accessToken, paymentId: id })
    });
    if (!sync.ok) console.error('Valto payment sync failed', await sync.text());
  } catch (e) {
    console.error('Valto payment sync error', e);
  }
